...
void mnbrak(float *lim1, float *lim2, float *lim12, float *flim1, float *flim2, float *flim12,float (*func)(float)){ 
	...
	Point u in interpolated by parabolic interpolation or by default magnification 
...
void Powell(float p[3], float ** unitVector,int n, float ftol, int *iter, float* fmin,float(*func) (float [3])){
	...
	/* Minimize function func (MI)  by research of the best transform in 3 dimensions.
	In this program, this function maximizes the negative MI with the best transformation (Tx,Ty,Rz).
	p is a initial transform and unitVector is a nxn unit matrix. min is returned as p and is found with
	a tolerance ftol of vale TOL with a limited number of iterations ITMAX. Value function of min is returned as fmin	

	Extrapolated point pextrapolated, initial point p0 and their function value fextrapolated and f0 defines criteria
	to keep old directions better than update the directions.
	*/

	int i = 0,ibig = 0,j= 0; 
	float del = 0.0f,t = 0.0f;
	float f0 = 0.0f, fextrapolated= 0.0f; // initial of function value and extrapolated function value
	float p0[3] = {0.0f,0.0f,0.0f} ,pextrapolated[3] = {0.0f,0.0f,0.0f};
