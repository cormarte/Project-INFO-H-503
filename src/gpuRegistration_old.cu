...
texture<unsigned char, cudaTextureType2D, cudaReadModeElementType> texReferenceImage;
...
__global__ void gpuGlobalHistogram1D(const unsigned int width, const unsigned int height, HistogramType* histogram) {
	...
	if (x < width && y < height) {
		...
		atomicAdd(&histogram[tex2D(texReferenceImage, x, y)], 1);
	}
}
...
Image gpuRegister(const Image& hostFloatingImage, const Image& hostReferenceImage) {
	...
	//CHECK(cudaBindTexture2D(NULL, &texReferenceImage, devReferenceImage, &channelDesc, width, height, imagePitch));

	cudaArray* cuArray;
	cudaMallocArray(&cuArray, &channelDesc, width, height);
	cudaMemcpyToArray(cuArray, 0, 0, hostReferenceImage.pixels, width*height*sizeof(unsigned char), cudaMemcpyHostToDevice);

	texReferenceImage.filterMode = cudaFilterModePoint;
	texReferenceImage.addressMode[0] = cudaAddressModeClamp;
	texReferenceImage.addressMode[1] = cudaAddressModeClamp;
	texReferenceImage.normalized = false;

	CHECK(cudaBindTextureToArray(&texReferenceImage, cuArray, &channelDesc));
